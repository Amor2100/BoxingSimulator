#include "Misc.h"
#include "Graphics/Shader/DefaultModelShader.h"
...
// 必要なバッファの設定
void DefaultModelShader::SetBuffers(const RenderContext& rc, const std::vector<Model::Node>& nodes, const ModelResource::Mesh& mesh)
{
	// メッシュ用定数バッファ更新
	CbMesh cbMesh;
...
// サブセット単位で描画
void DefaultModelShader::DrawSubset(const RenderContext& rc, const ModelResource::Subset& subset)
{
	CbSubset cbSubset;
