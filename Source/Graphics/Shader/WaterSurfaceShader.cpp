#include "Misc.h"
#include "Graphics/Shader/WaterSurfaceShader.h"
...
WaterSurfaceShader::WaterSurfaceShader(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\WaterSurfaceVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\WaterSurfacePS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
void WaterSurfaceShader::Begin(const RenderContext& rc)
{
	...
	memcpy_s(cbScene.pointLightData, sizeof(cbScene.pointLightData),
		rc.pointLightData, min(sizeof(cbScene.pointLightData), sizeof(rc.pointLightData)));
	...
	{
		rc.skyboxData.skybox,
		rc.shadowMapData.shadowMap,
	};
	rc.deviceContext->PSSetShaderResources(2, ARRAYSIZE(srvs), srvs);

}

// 必要なバッファの設定
void WaterSurfaceShader::SetBuffers(const RenderContext& rc, const std::vector<Model::Node>& nodes, const ModelResource::Mesh& mesh)
{
	// メッシュ用定数バッファ更新
	CbMesh cbMesh;
...
// サブセット単位で描画
void WaterSurfaceShader::DrawSubset(const RenderContext& rc, const ModelResource::Subset& subset)
{
	CbSubset cbSubset;
	cbSubset.materialColor = subset.material->color;
	cbSubset.ka = subset.material->waterSurface.phong.ka;
	cbSubset.kd = subset.material->waterSurface.phong.kd;
	cbSubset.ks = subset.material->waterSurface.phong.ks;
	cbSubset.shiness = subset.material->waterSurface.phong.shiness;

	cbSubset.wave1 = subset.material->waterSurface.waveScroll1;
	cbSubset.wave2 = subset.material->waterSurface.waveScroll2;
	cbSubset.wave1.x *= rc.timer;
	cbSubset.wave1.y *= rc.timer;
	cbSubset.wave2.x *= rc.timer;
	cbSubset.wave2.y *= rc.timer;
